...
0.85 for English and 0.85 for Korean (including Korean-to-English code queries).
Fast English evidence recall was 0.90 and fast Hit@5 was 0.85; the matched
...
verification/session loading. Reproduce the 10 MiB / 1,000-file fast benchmark
with at least 20 steady-state runs:
...
On the measured four-core ARM host, the 20 steady-state runs had 25.31 s median
and 26.02 s p95 end-to-end time, 10.24 s median batch inference, and about
733,456 KiB p95 sampled peak RSS. Memory was under the 1.5 GiB goal; the 15 s
end-to-end and 10 s 128-passage inference goals were **not met**. The search is
functional, but the default bounded fast path should not be described as meeting
those latency targets for a 10 MiB / 1,000-file corpus. Details and raw records
